...
if (( $(echo "$brightness <= 0.1" |bc -l) ))  # If at lowest brightness
then exit  # Exit and do nothing
fi

# Getting (primary) monitor
monitor=($(xrandr --listmonitors))  # Getting raw output
export monitor=$(echo ${monitor[5]})  # Get index position of word monitor

# Computing new brightness
subtracted=$(echo $brightness - 0.1 | bc)  # Adding 0.1 to existing brightness
$( > $path) # Empty the file
$(echo "$subtracted" > $path)  # Add and save with new brightness

xrandr --output $monitor --brightness $subtracted  # Apply brightness
